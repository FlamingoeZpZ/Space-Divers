Shader "Unlit/HeightToNormal"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                const float me = tex2D(_MainTex,i.uv1).x;
                ...
                const float3 perp1 = normalize(cross(i.normals,temp));
                float3 perp2 = normalize(cross(i.normals,perp1));

                //use the basis to move the normal i its own space by the offset
                const float3 normalOffset = -_NormalIntensity * (((n - me) - (s - me)) * perp1 + ((e - me) - (w - me)) * perp2);
                norm += normalOffset;
                norm = normalize(norm);
